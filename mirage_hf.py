...
class MIRAGESegHead(nn.Module):
    def __init__(self, num_classes, in_dim=768, hidden=96, patch_grid=(16, 16)):
        super().__init__()
        self.patch_grid = patch_grid
        h, w = patch_grid
        # Each token unfolds to 8×8 spatial region
        self.proj = nn.Linear(in_dim, hidden * 64)  # 768 -> 6144

        self.blocks = nn.Sequential(*[ConvNeXtBlock(hidden) for _ in range(4)])
        self.upsample = nn.Upsample(scale_factor=4, mode='bilinear')
        self.head = nn.Conv2d(hidden, num_classes, 1)

    def forward(self, tokens):
        # tokens: (B, 257, 768)
        x = tokens[:, :-1, :]  # drop global -> (B, 256, 768)
        B = x.shape[0]
        h, w = self.patch_grid

        x = self.proj(x)  # (B, 256, 6144)
        # Reshape: each token becomes 8×8 spatial with 96 channels
        x = rearrange(x, 'b (h w) (ph pw c) -> b c (h ph) (w pw)',
                      h=h, w=w, ph=8, pw=8)  # (B, 96, 128, 128)

        x = self.blocks(x)
        x = self.upsample(x)  # (B, 96, 512, 512)
...
class MIRAGESegmenter(nn.Module):
    def __init__(self, num_classes=12, freeze_encoder=True):
        super().__init__()
        self.encoder = MIRAGEhf.from_pretrained("j-morano/MIRAGE-Base")
        self.head = MIRAGESegHead(num_classes, in_dim=768, patch_grid=(16, 16))
    ...
    def forward(self, x):
        # x: (B, 1, 512, 512) grayscale B-scan
        tokens = self.encoder({'bscan': x})  # (B, 257, 768)
